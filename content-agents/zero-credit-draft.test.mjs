...
test('existing default topic returns an exact curated text draft without paid AI calls', async () => {
  ...
  assert.equal(draft.campaign.quality.scriptureAccurate, false)
  assert.equal(draft.campaign.quality.biblicalConsistency, false)
  assert.equal(draft.campaign.quality.gospelCentered, false)
