...
const candidates=[...map.values()].map(c=>{const csi=c.evidence.find(e=>e.source==="creator_search_insights");const gap=Boolean(csi?.content_gap===true&&(csi?.evidence_url||csi?.evidence_id||csi?.captured_at));let score=30+(gap?30:0)+(c.evidence.some(e=>e.source==="web")?5:0)+(c.query.split(/\s+/).length>=3?5:0);return {...c,score,verified_content_gap:gap,content_gap_status:gap?"VERIFIED":"UNVERIFIED",production_eligible:gap||c.evidence.length>=2};}).sort((a,b)=>b.score-a.score);
const result={version:cfg.version,generated_at:new Date().toISOString(),safeguards:{content_gap_claim_requires_tiktok_evidence:true,scripture_accuracy_required:true,master_ready_required:true,publishing_gate_unchanged:true},stats:{candidates:candidates.length,verified_content_gaps:candidates.filter(x=>x.verified_content_gap).length,production_eligible:candidates.filter(x=>x.production_eligible).length},candidates};
