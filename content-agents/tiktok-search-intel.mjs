...
const candidates=[...map.values()].map(c=>{const csi=c.evidence.find(e=>e.source==="creator_search_insights");const gap=Boolean(csi?.content_gap===true&&(csi?.evidence_url||csi?.evidence_id));let score=30+(gap?30:0)+(c.evidence.some(e=>e.source==="web")?5:0)+(c.query.split(/\s+/).length>=3?5:0);return {...c,score,verified_content_gap:gap,content_gap_status:gap?"VERIFIED":"UNVERIFIED",production_eligible:gap||c.evidence.length>=2};}).sort((a,b)=>b.score-a.score);
