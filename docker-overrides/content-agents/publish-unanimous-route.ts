...
function releaseHash(payload:any){
  return crypto.createHash('sha256').update(JSON.stringify(canonicalize(payload))).digest('hex')
}
function approvalKey(contentHash:string,masterHash:string,agentId:string){
  return `one-million-souls:v59:approval:${contentHash}:${masterHash}:${agentId}`
}
function certificateKey(contentHash:string,masterHash:string){
